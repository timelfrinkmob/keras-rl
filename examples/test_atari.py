...
if POL == 'eps':
    ...
    policy = LinearAnnealedPolicy(EpsGreedyQPolicy(), attr='eps', value_max=1., value_min=.01, value_test=.01,
                              nb_steps=(nb_steps/2))
else:
...
dqn = DQNAgent(model=model, nb_actions=nb_actions, heads=bsheads, memory=memory, nb_steps_warmup=50000, processor=processor, train_interval=4,
                   target_model_update=10000, delta_clip=1., batch_size=32, gamma=.99, policy=policy,enable_double_dqn=bootstrap)
